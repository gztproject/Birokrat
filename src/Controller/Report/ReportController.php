...
class ReportController extends AbstractController {
	...
	private function getOverviewReport(?String $organizationId, ?String $dateFrom, ?String $dateTo, EntityManagerInterface $em): array {
		...
		] )->from ( 'App\Entity\Konto\Konto', 'k', 'k.id' )->leftJoin ( 'App\Entity\Konto\KontoCategory', 'kc', 'WITH', 'k.category = kc.id' )->leftJoin ( 'App\Entity\Transaction\Transaction', 't', 'WITH', '(t.debitKonto = k.id OR t.creditKonto = k.id)' )->where ( 't.hidden = 0' );
	...
	private function getKontoReport(?String $organizationId, ?String $dateFrom, ?String $dateTo, EntityManagerInterface $em): array {
		...
		->leftJoin ( 'App\Entity\Transaction\Transaction', 't', 'WITH', '(t.debitKonto = k.id OR t.creditKonto = k.id)' )->where ( 't.id IS NOT NULL' );//->andWhere ( 't.hidden = 0' );
		if ($organizationId !== null)
			$qb->andWhere ( 't.organization = :orgId' );
		if ($dateFrom !== null)
			$qb->andWhere ( 't.date >= :dateFrom' );
	...
	private function getBilancaReport(?String $organizationId, ?String $date, EntityManagerInterface $em): Bilanca {
		...
		->leftJoin ( 'App\Entity\Transaction\Transaction', 't', 'WITH', 't.debitKonto = k.id OR t.creditKonto = k.id' )->Where ( 't.hidden = 0' );
		if ($organizationId !== null)
			$qb->andWhere ( 't.organization = :orgId' );
		// if ($dateFrom !== null)
		// $qb->andWhere ( 't.date >= :dateFrom' );
	...
	private function getTurnoutReport(?String $organizationId, ?String $dateFrom, ?String $dateTo, EntityManagerInterface $em): Turnout {
		...
		->leftJoin ( 'App\Entity\Konto\KontoCategory', 'kc', 'WITH', 'k.category = kc.id' )
		->leftJoin ( 'App\Entity\Transaction\Transaction', 't', 'WITH', 't.sum >= 0 AND (t.debitKonto = k.id OR t.creditKonto = k.id)' )->Where ( 't.hidden = 0' );
		if ($organizationId !== null)
			$qb->andWhere ( 't.organization = :orgId' );
		if ($dateFrom !== null)
			$qb->andWhere ( 't.date >= :dateFrom' );
